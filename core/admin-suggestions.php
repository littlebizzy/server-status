 ...
 */
final class SVRSTS_Admin_Suggestions {
	...
	public static function instance() {
		...
		if (!isset(self::$instance))
			self::$instance = new SVRSTS_Admin_Suggestions;
	...
	*/
	private function __construct() {
		...
		$timestamp = (int) get_option( 'svrsts_dismissed_on' );

		if ( empty( $timestamp ) || ( time() - $timestamp ) > (90 * 86400) ) {

			// Check AJAX submit
			if (defined('DOING_AJAX') && DOING_AJAX) {
				add_action( 'wp_ajax_svrsts_dismiss', array(&$this, 'dismiss'));
...
<script type="text/javascript">jQuery(function($) {	$(document).on('click', '.svrsts-dismiss .notice-dismiss', function() { $.post(ajaxurl, {'action':'svrsts_dismiss','nonce':$(this).parent().attr('data-nonce')}); }); });</script>
	<?php }
	...
	public function dismiss() {
		if (!empty($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'svrsts-dismiss')) {
			update_option('svrsts_dismissed_on', time(), true);
		}
	}
	...
	public function admin_notices() {
		...
		$plugin_data = get_plugin_data( SVRSTS_FILE );

		?><div class="svrsts-dismiss notice notice-success is-dismissible" data-nonce="<?php echo wp_create_nonce('svrsts-dismiss'); ?>">
