...
class SVRSTS_Admin_Dashboard {
	...
	public static function widget() {
		...
		$items = apply_filters('srvsts_items', array(
			...
			'Memory Usage'							=> esc_html($memory['usage']).' ('.esc_html($memory['percent']).'%) of '.esc_html($memory['limit']).' (actual limit)',
			...
			'Opcache Size (Max Files, Revalidate)' 	=> (empty($opcache_size)? 0 : esc_html(SVRSTS_Core_Status::format_bytes_ini_val($opcache_size))).' ('.(empty($max_accelerated_files)? 0 : esc_html($max_accelerated_files)).', '.(empty($revalidate_freq)? 'n/a' : esc_html($revalidate_freq)).' secs)',
