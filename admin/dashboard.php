...
class SVRSTS_Admin_Dashboard {
	...
	public static function widget() {
		...
		$items = apply_filters('srvsts_items', array(
			...
			'Disk Space'							=> esc_html($disk_space['free']).' GB (free) of '.$disk_space['total'].' GB (total)',
			'Database Size'							=> esc_html(SVRSTS_Core_Status::database_size()).' MB',
			...
			'Default Timezone'						=> esc_html(date_default_timezone_get()),
			...
			'Memory Usage'							=> esc_html($memory['usage']).'M ('.esc_html($memory['percent']).'%) of '.esc_html($memory['limit']).' (actual PHP limit)',
			...
			'Page Cache + Object Cache'				=> ((defined('WP_CACHE') && WP_CACHE)? 'Enabled' : 'Disabled').' + '.($_wp_using_ext_object_cache? 'Enabled' : 'Disabled'),
		));
